...
#http://easdps01.esac.esa.int:8000/DbView
#https://euclidsoc.esac.esa.int:8300/{status,jobs,files}
#http://archives.esac.esa.int/esasky-beta

#+begin_src plantuml :file procelem_plantuml.png
@startuml
testdot
@enduml
#+END_SRC
...
*** TODO Euclid Operations Main Entities
...
- QLA Processing/Algorithmic Functions :: It is formed by the set of libraries and processing elements that perform the different analysis on the input data.
...
** The QLA Processing Framework (QPF)
...
The QPF will be composed of a set of components that take the incoming external events and data, check their basic consistency, perform a specific orchestration based on the type of incoming data, and launches the neccessary processing tasks to end up with the set of diagnostics, statistics and output parameters to evaluate the validity and quality of the products.  In section [[QPF Components]] a detailed description of them is provided.
...
*** Mapping between QPF and QLA components                       :noexport:
...
*** QPF Use Cases                                                :noexport:

**** System Launch and Initialisation
***** Description
The system application is executed in all the hosts that compose the framework network, the HMI is launched, in the main host, and the entire set of Components start their execution.  The components communications are established, and all of them reach the INITIALISED State.
...
- Main host and processing nodes network connectivity is ready

***** Nominal actions sequence
On every host in the hosts network:
1. The Application component is launched, which creates the Deployer object
2. The Deployer parses the command line and decides whether the configure must be read from the DB or from a provided configuration file
3. The Deployer processes the configuration
4. The Deployer creates the components to be delivered at host
5. Components are configured according to current configuration
6. The different components created are launched
7. The components launched are initialized
8. If the HMI component is foreseen to be deployed in the host, create the HMI component and launch it.

#+begin_src plantuml :file system-launch_plantuml.png
@startuml
!include "./config.plu"
...
#+begin_src plantuml :file system-start_plantuml.png
@startuml
!include "./config.plu"
...
#+begin_src plantuml :file external-data-injection_plantuml.png
@startuml
!include "./config.plu"
title Use Case: External Data Injection
participant EvtMng
...
deactivate EvtMng
DataMng -> LA: downloadProducts
DataMng -> DataMng: changeOrigin(LocalArchive)
DataMng -> DB: registerProducts
...
**** Processing Data from User-selected Input Folder

#+begin_src plantuml :file user-selected-input_plantuml.png
@startuml
!include "./config.plu"
!define SEND(a,b,c)  a -[#blue]> b: c
!define START1(a) activate a #lightyellow
!define START2(a) activate a #bisque
!define STOP(a) deactivate a
title Use Case: Processing Inbox Data
...
participant "Local\nArchive" as LA #cyan
database "Database" as DB #cyan
Op -> HMI: select input folder
HMI -> HMI: generate INDATA msgs
loop for each message/product
HMI -> HMIpxy: send MSG_INDATA
SEND(HMIpxy, EvtMng, MSG_INDATA)
START1(EvtMng)
EvtMng -> EvtMng: changeOrigin(InboxArea)
SEND(EvtMng, DataMng, MSG_INDATA)
START1(DataMng)
/' EvtMng --> DB: <font color="grey"><size:10><i>saveToDB(<b>MSG_INDATA</b>)</i></size></font> '/
STOP(EvtMng)
DataMng -> LA: saveProducts
DataMng -> DataMng: changeOrigin(LocalArchive)
DataMng -> DB: registerProducts
SEND(DataMng, TaskOrc, MSG_INDATA)
START1(TaskOrc)
/' DataMng --> DB: <font color="grey"><size:10><i>saveToDB(<b>MSG_INDATA</b>)</i></size></font> '/
STOP(DataMng)
TaskOrc -> TaskOrc: checkRules
...
#+begin_src plantuml :file proc-rule_plantuml.png
@startuml
!include "./config.plu"
...
**** Processing Products fro User-selected Input Folder - Complete Sequence

#+begin_src plantuml :file proc-user-input_plantuml.png
@startuml
!include "./config.plu"
show footbox
!define SEND(a,b,c)  a -[#blue]> b: c
/'!define REGISTER(a,c)  a -[#gray]-> DB: <font color="grey"><size:10><i>saveToDB(<b> c </b>)</i></size></font>'/
/'!definelong SEND(a,x,c)
a -[#blue]> x: c
a -[#gray]-> DB: <font color="grey"><size:10><i>saveToDB(<b> c </b>)</i></size></font>
!enddefinelong'/
!define START1(a) activate a #PapayaWhip
!define START2(a) activate a #PeachPuff
!define STOP(a) deactivate a
!definelong DO(a,x)
a -> a: x
START2(a)
...
title Processing Products fro User-selected Input Folder - Complete Sequence

actor Operator as Op

box "Main Node" #MintCream
participant HMI
participant "HMI proxy" as HMIpxy
participant EvtMng
participant DataMng
...
box "Processing Node" #LavenderBlush
participant TaskAg
participant ProcElem
participant "Docker\nContainer" as Docker
end box

box "Storage and Registration" #azure
participant "Local\nArchive" as LA #lightcyan
participant "Shared\nStorage" as SH #lightcyan
database "Database" as DB #cyan
end box
...
STOP(EvtMng)
DataMng -> LA: saveProducts
DO(DataMng,changeOrigin(LocalArchive))
DataMng -> DB: registerProducts
SEND(DataMng, TaskOrc, MSG_INDATA)
START1(TaskOrc)
STOP(DataMng)
DO(TaskOrc, checkRules)
note left : in case of rule firing,\ncreate Processing Task
ref over TaskOrc: rule fired
end loop

group rule fired
DO(TaskOrc, buildProcessingTask)
DataMng -> SH: placeProductsAtSharedArea
DO(TaskOrc, chageOrigin(SharedAreaIn))
SEND(TaskOrc, TaskMng, MSG_TASK_PROC)
START1(TaskMng)
SEND(TaskOrc, DataMng, MSG_TASK_PROC)
STOP(TaskOrc)
DO(TaskMng, evaluateTask)
DO(TaskMng, selectAgent)
SEND(TaskMng, TaskAg, MSG_TASK_PROC)
STOP(TaskMng)
START1(TaskAg)
DO(TaskAg, processTask)
create ProcElem
TaskAg -> ProcElem: provideTaskData
START1(ProcElem)
DO(ProcElem, buildDockerOrder)
create Docker
ProcElem -> Docker: runTask
...
ProcElem -> Docker: monitorize task
START2(ProcElem)
STOP(ProcElem)
TaskAg -> ProcElem: requestRuningTaskInfo
START2(TaskAg)
TaskAg <-- ProcElem: runingTaskInfo
STOP(TaskAg)
START1(TaskMng)
SEND(TaskAg, TaskMng, MSG_TASK_RES)
note left: <size:8>MSG_TASK_RES would be</size>\n<size:8>sent also to HMIpxy through</size>\n<size:8>EvtMng for display in HMI</size>
/'group display on HMI
SEND(TaskMng,EvtMng, MSG_TASK_RES)
START1(EvtMng)
SEND(EvtMng,HMIpxy, MSG_TASK_RES)
START1(HMIpxy)
STOP(EvtMng)
HMIpxy -> HMI: provideTaskStatusInfo
START1(HMI)
DO(HMI,display updated task info)
STOP(HMIpxy)
STOP(HMI)
end group'/
end group

Docker -> SH: saveOutputs
STOP(Docker)
DO(ProcElem, generateListOfProducts)
destroy Docker
TaskAg <-- ProcElem: provideOutputsInfo
destroy ProcElem
SEND(TaskAg, TaskMng, MSG_TASK_RES)
STOP(TaskAg)
TaskMng -> DB: updateTaskInfo
...
START1(DataMng)
SEND(TaskMng, DataMng, MSG_TASK_RES)
STOP(TaskMng)
DataMng -> LA: saveProducts
DO(DataMng,changeOrigin(LocalArchive))
DataMng -> DB: registerOutputs
...
#+begin_src plantuml :file _plantuml.png
@startuml
!include "./config.plu"
title Use Case: System Launch and Initialisation
participant App
...
**** System Maintenance
The QPF system adninistrator realizes a series of tasks for maintenance of the QPF system, including monitoring of the disks space, DB cleaning/backup, addition/substitution of hosts into the system, etc.
...
**** System Internal Reporting
The QPF internally generates log and monitoring information.

**** System Start
The QPF is started, spawning its components, creating the appropriate connections among them, performing the neccessary internal checks, connecting to the internal DB, and processing the selected configuration.

**** System Shutdown
The QPF saves its internal state, closes connections, and deletes all temporary resources.

**** System Configuration
This includes the modification, from the HMI or using an external tool, of the Configuration file of the QPF.

**** System Re-configuration
This includes the modification of the QPF configuration, if needed, and the restart of the system to activate the newly selected configuration.
...
**** Product Processing
This is an autonomous process that is performed by the QPF upon external products procurement, or retrieval of output products generated in a previous product processing.  The rules included in the QPF active configuration are evaluated for each of these products, and in case one of them is fired, the corresponding processor is launched according to its configured behaviour.  Once the processing is finished, the output products are ingested in the local archive and in the EAS @ SOC, and the rules are again checked against them.

This use case is specialised by the Container-based and the Swarm-based Product Processing use cases.

Includes:
- Product Ingestion

**** Product Re-processing
The user of the system selects one or more products from the local archive, and requests their processing, according to the Product Processing use case.

Includes:
- Product Processing

**** User Products Processing
The user provides to the system a folder where a set of products are ready to be processed.  The QPF loads them and proceeds as per in Product Processing use case.

Includes:
- Product Processing

**** Swarm-based Product Processing
The input product is provided to the first swarm service element in the corresponding configured swarm for that type of products.  Once the swarm service has access to this product, this is processed.  Once the processing is finished, the swarm service send notification to the system about the generated output products, for their retrieval.

**** Container-based Product Processing
A new container is created and the corresponding configured processor for that type of product is launched, with the input product to be processed.  Once the processing is finished, the outputs are retrieved.

**** Product Ingestion
The generated output products that need to be persisted are stored in the EAS storage, and their metadata is registered into the EAS database, for further access and retrieval.

**** Products External Analysis
The user selects one or more products from the local archive, and requests the openning of an external tool for their analysis.
...
- PCRE2 :: File name templates will be compiled against PCRE2 library.
...
#+begin_src plantuml :file db-config_plantuml.png
@startuml
...
#+begin_src plantuml :file db-products_plantuml.png
@startuml
!define table(x) class x << (T,#FFDDAA) >>
!define primary_key(x) #<b>x</b>
!define field_type(x) <i>x</i>
...
#+begin_src plantuml :file db-tasks_plantuml.png
@startuml
!define table(x) class x << (T,#FFDDAA) >>
!define primary_key(x) #<b>x</b>
!define field_type(x) <i>x</i>
...
#+begin_src plantuml :file db-msg_plantuml.png
@startuml
!define table(x) class x << (T,#FFDDAA) >>
!define primary_key(x) #<b>x</b>
!define field_type(x) <i>x</i>
...
#+begin_src plantuml  :file db-alarms_plantuml.png
@startuml
!define table(x) class x << (T,#FFDDAA) >>
!define primary_key(x) #<b>x</b>
!define field_type(x) <i>x</i>
...
**** Classes Specification                                      :noexport:

***** Database related                                         :noexport:

#+begin_src plantuml :file noexport.png
@startuml
'!define table(x) class x << (T,#FFDDAA) >>
'!define primary_key(x) #<b>x</b>
'!define field_type(x) <i>x</i>
...
*** Specification - TO BE UPDATED                                  :noexport:
**** QPF Database Specification                                 :noexport:
...
**** Current QPF Configuration                                  :noexport:
***** STARTED JSON
****** "general":
******* "app_name": "QPF",
******* "app_version": "0.1",
...
***** DEFERRED Relational Tables
CLOSED: [2015-10-09 vie 08:45]
****** config_general
| Parameter   | Content         |
| [string]    | [string]        |
|-------------+-----------------|
...
*** TODO Issues [2/3]                                            :noexport:
**** DONE [#A] Remote log chunks are received in =DATA_INFO= messages, but they are not saved to disk
CLOSED: [2015-10-05 lun 11:44]
**** DONE [#A] Database structure is obsolete, so INSERTs and other operations fail
CLOSED: [2015-10-05 lun 11:44]
**** TODO [#A] JSONCpp library shows some bugs from time to time, this should be avoided by using our own SDC library, and restricting JSON just to I/O operations

*** TODO Functionalities to be included [8/14]                   :noexport:
**** DONE [#A] Correct DB structure so the incoming data and overall information can be stored
CLOSED: [2015-10-05 lun 11:43]
**** DONE [#A] Ensure the received remote log chunks are stored to their corresponding remote log files (under /rlog)
CLOSED: [2015-10-05 lun 11:43]
**** DONE [#B] Prepare an infrastructure for product downloading
CLOSED: [2016-04-04 lun 09:14]
**** DONE [#B] Implement output products registering and archival
CLOSED: [2016-01-29 vie 08:48]
**** DONE [#A] Registration of tasks status
CLOSED: [2015-09-30 mié 13:23]
**** DONE [#A] Checking of last execution tasks status
CLOSED: [2015-09-30 mié 13:23]
**** DONE [#A] Allow deployment of components in different hosts
CLOSED: [2015-09-30 mié 13:23]
**** DONE [#B] Output files registration
CLOSED: [2015-09-30 mié 13:23]
**** TODO [#A] Ensure the HMI monitors the remote log directory and they are shown in the HMI
**** TODO [#B] Implement =MONIT_RQST= and =MONIT_INFO= messages
**** TODO [#B] Implement Commanding messages to handle running processors
**** TODO [#B] Pause/Resume/Stop of tasks
**** STARTED [#C] Implement preferences for HMI
...
** TBDs, TBCs and TODO lists                                      :noexport:
