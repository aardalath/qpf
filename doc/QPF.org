...
***** Description
The system application is executed in all the hosts that compose the framework network, the HMI is launched, in the main host, and the entire set of Components start their execution.  The components communications are established, and all of them reach the OPERATIONAL State.
...
- Main host and processing nodes connectivity is ready
-
