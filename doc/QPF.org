...
*** TODO Issues [0/2]
**** TODO [#A] Remote log chunks are received in DATA_INFO messages, but they are not saved to disk
**** TODO [#A] Database structure is obsolete, so INSERTs and other operations fail


*** TODO Functionalities to be included [4/14]
**** TODO [#A] Correct DB structure so the incoming data and overall information can be stored
**** TODO [#A] Ensure the received remote log chunks are stored to their corresponding remote log files (under /rlog)
